...
public class TradingThread implements Runnable {
    ...
    private void processLevelCrossings(TradingPairEntity pair, BigDecimal oldPrice, BigDecimal newPrice) {
        BigDecimal step = pair.getStartPrice().multiply(pair.getGridLevelPercentage())
        ...
        if (newLevelPrice != null && (currentLevelPrice == null || !currentLevelPrice.equals(newLevelPrice))) {
            log.info("Level crossing detected for {}: {} -> {} (Level: {})",
                     pair.getSymbol(), oldPrice, newPrice, newLevelPrice);
    ...
    public BigDecimal findGridLevelPrice(BigDecimal startPrice, BigDecimal step,
        ...
        if ((candidate.compareTo(oldPrice) > 0 && candidate.compareTo(newPrice) <= 0) ||
            (candidate.compareTo(oldPrice) < 0 && candidate.compareTo(newPrice) >= 0)) {
            return candidate;
        }
